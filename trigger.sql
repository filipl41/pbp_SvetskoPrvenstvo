...
for each row 
begin 
if new.gost = (select idDrzava from sudija s where s.idSudija = new.idSudije) or new.domacin = (select idDrzava from sudija s where s.idSudija = new.idSudije) 
then SIGNAL SQLSTATE '45000' SET message_text = 'Sudija ne moze biti iz iste zemlje kao timovi kojima sudi!'; 
end if; 
end $
